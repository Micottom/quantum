...
c2 = [[0, 1, 6, 1, 8, 0], [1, 0, 3, 3, 9, 8], [6, 3, 0, 8, 7 , 4], [1, 3, 8, 0, 9 , 8], [8, 9, 7, 9, 0, 9],[0, 8, 4, 8, 9, 0]]
beds = [1, 2, 3]
...
for d in range(numberOfDormitories):
    cqm.add_constraint(quicksum(x[(q,d)]
    for q in range(totalNumberOfSloths)) <= beds[d])

for i in range(totalNumberOfSloths):
    cqm.add_constraint(quicksum(x[(i,p)]
    for p in range(numberOfDormitories)) == 1)
